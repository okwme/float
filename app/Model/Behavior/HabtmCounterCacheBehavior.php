  ...
  protected function _setNewHabtmIds($model) {
    ...
    foreach ($this->settings[$model->alias] as $habtmAlias => $settings) {
      ...
      if(isset($this->_habtmIds[$model->alias][$model->id][$habtmAlias]) && 
        is_array($this->_habtmIds[$model->alias][$model->id][$habtmAlias])):
      if(isset($model->data[$habtmAlias]) && is_array($model->data[$habtmAlias])):
      // If there are old habtm ids merge them with the new ones
        //debug($this->_habtmIds[$model->alias][$model->id]);
        //debug($model->data);
        $foo = array_merge(
      ...
      $this->_habtmIds[$model->alias][$model->id][$habtmAlias] = $foo;
    endif;
    endif;
